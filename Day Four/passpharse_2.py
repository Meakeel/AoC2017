...
def validpassphrases(value):
    matrix = []
    i = 0
    matrix = value.splitlines()

    for y in range(len(matrix)):
        matrix[y] = [''.join(sorted(s)) for s in matrix[y].split()]
    ...
    for u in range(numrows):
        numPasswords = len(matrix[u])
        for z in range(numPasswords):
            count = matrix[u].count(matrix[u][z])
            if count > 1:
                isValid = False
        if isValid:
            total = total + 1
...
test1Input = "aa bb cc dd ee"
test1Result = validpassphrases(test1Input)
print(test1Result)

test2Input = "aa bb cc dd aa"
test2Result = validpassphrases(test2Input)
print(test2Result)

test3Input = "aa bb cc dd aaa"
test3Result = validpassphrases(test3Input)
print (test3Result)

resultInput = """pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
